    ...
    <section className="py-12">
      <Container>
        ...
        <div className="flex justify-center gap-4">
          {socials.map((s, i) => (
            <ScrollReveal key={s.name} animation="scale-in" delay={i * 100}>
              <a
                href={s.href}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={s.name}
                className={`flex h-16 w-16 items-center justify-center rounded-2xl border border-border/50 bg-background text-muted-foreground transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${s.color}`}
              >
                {s.icon}
              </a>
            </ScrollReveal>
